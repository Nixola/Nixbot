...
function newobject:SetText(t)
	...
	if maxw > 0 then
		for k, v in ipairs(self.formattedtext) do
			...
			--
			local width
			local runs, w = pcall(v.font.getWidth, v.font, data)
			if runs then width = w
			else width=800-16
			end
			--
			...
			if width > maxw then
				...
				for n=1, #data do	
					...
					--
					local itemw
					local runs, w = pcall(v.font.getWidth, v.font, item)
					if runs then itemw = w
					else itemw=800-16
					end
					--
	...
	for k, v in ipairs(textdata) do
		...
		if type(text) == "string" then
			...
			--
			local width
			local runs, w = pcall(v.font.getWidth, v.font, text)
			if runs then width = w
			else width=800-16
			end
			--
	...
	for k, v in ipairs(textdata) do
		...
		if inlist then
			if (y + v.y) <= (list.y + list.height) and self.y + ((v.y + theight)) >= list.y then
				...
				pcall(love.graphics.print,text, x + v.x, y + v.y)--UGLY fix
			end--
		else
			love.graphics.setFont(font)
			if shadow then
